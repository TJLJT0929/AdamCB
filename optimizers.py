...
def adam_update(model, G_t_hat, m_t, v_t, v_hat, t, lr, betas, lambda_, eps, device, q):
...
def amsgrad_update(model, G_t_hat, m_t, v_t, v_hat, t, lr, betas, lambda_, eps, device, q):
    beta1 = betas[0] * lambda_ ** (t-1)
    beta2 = betas[1]
    alpha_t = lr / t**0.5
    with torch.no_grad():
